...
public enum ReviewAction
{
    ...
    ApproveEvidence = 3,
    RejectEvidence = 4,
    RequestEvidenceRevision = 5,
    ApproveApplication = 6,
    RejectApplication = 7,
    RecommendNextLevel = 8
}
...
public sealed class Application : AggregateRoot<Guid>, IAuditableEntity
{
    ...
    public Application(ApplicationType type) : this()
    {
    ...
    {
        if (Status != ApplicationStatus.Draft && Status != ApplicationStatus.NeedsRevision)
            throw new Exception("Hồ sơ phải ở trạng thái Soạn thảo hoặc Cần bổ sung.");

        if (ApplicationType == ApplicationType.Individual && StudentId == null)
            throw new Exception("Hồ sơ cá nhân bắt buộc phải có thông tin sinh viên.");

        if (ApplicationType == ApplicationType.Collective && UnitId == null)
            throw new Exception("Hồ sơ tập thể bắt buộc phải có thông tin đơn vị.");

        Status = ApplicationStatus.Submitted;
        SubmittedAt = DateTime.UtcNow;
        AddReviewLog(userId, ReviewAction.Submit, "Đã nộp hồ sơ");
    }

    public void StartReview(Guid reviewerId)
    {
        if (Status != ApplicationStatus.Submitted)
            throw new Exception("Chỉ bắt đầu xét duyệt khi hồ sơ đã nộp.");
            
        Status = ApplicationStatus.UnderReview;
        AddReviewLog(reviewerId, ReviewAction.StartReview, "Bắt đầu xét duyệt");
    }

    public void RequestRevision(Guid reviewerId, string note)
    {
        if (Status != ApplicationStatus.UnderReview)
            throw new Exception("Chỉ yêu cầu bổ sung khi đang xét duyệt.");
            
        Status = ApplicationStatus.NeedsRevision;
        AddReviewLog(reviewerId, ReviewAction.RequestEvidenceRevision, note);
    }

    public void Approve(Guid reviewerId, string? note = null)
    {
        if (Status != ApplicationStatus.UnderReview && Status != ApplicationStatus.Submitted)
            throw new Exception("Chỉ duyệt hồ sơ khi đã nộp hoặc đang xét duyệt.");
        
        Status = ApplicationStatus.Approved;
        AddReviewLog(reviewerId, ReviewAction.ApproveApplication, note ?? "Đã duyệt hồ sơ");
    }
    ...
    public void Reject(Guid reviewerId, string reason)
    {
        ...
        AddReviewLog(reviewerId, ReviewAction.RejectApplication, $"Từ chối hồ sơ: {reason}");
    }

    public void RecommendForNextLevel(Guid reviewerId)
    {
        if (Status != ApplicationStatus.Approved)
            throw new Exception("Hồ sơ phải được duyệt mới có thể đề xuất lên cấp trên.");
            
        RecommendedForNextLevel = true;
        AddReviewLog(reviewerId, ReviewAction.RecommendNextLevel, "Đề xuất xét duyệt cấp trên");
    }

    private void AddReviewLog(Guid reviewerId, ReviewAction action, string note)
    {
...
public sealed class ApplicationCriterion
{
    public Guid ApplicationId { get; private set; }
    public Application Application { get; private set; } = null!;
    
    public Guid CriterionId { get; private set; }
    public Criterion Criterion { get; private set; } = null!;
    
    public DateTime SelectedAt { get; private set; } = DateTime.UtcNow;
}
```
...
{
    Task<Guid> CreateIndividualApplicationAsync(Guid campaignId, CancellationToken ct = default);
    Task<Guid> CreateCollectiveApplicationAsync(Guid campaignId, Guid unitId, string unitName, CancellationToken ct = default);
    ...
    ILogger<ApplicationService> logger) : IApplicationService
{
    ...
    public async Task<Guid> CreateIndividualApplicationAsync(Guid campaignId, CancellationToken ct = default)
    {
        ...
        var app = new Application(ApplicationType.Individual)
        {
            CampaignId = campaignId,
            StudentId = CurrentUserId,
            UnitId = currentUser.UnitId,
            ApplicationCode = $"SV5T-IND-{DateTime.UtcNow.Year}-{Guid.NewGuid().ToString("N")[..6].ToUpperInvariant()}"
        };
    ...
    public async Task<Guid> CreateCollectiveApplicationAsync(Guid campaignId, Guid unitId, string unitName, CancellationToken ct = default)
    {
        var campaign = await campaignRepository.GetByIdAsync(campaignId, tracking: false, ct)
            ?? throw new UseCaseException(ApplicationErrorKind.NotFound, "Chiến dịch không tồn tại.");
        ...
        var existing = await applicationRepository.GetByUnitAndCampaignAsync(campaignId, unitId, tracking: false, ct);
        if (existing is not null) return existing.Id;

        var app = new Application(ApplicationType.Collective)
        {
            CampaignId = campaignId,
            UnitId = unitId,
            UnitNameSnapshot = unitName,
            ApplicationCode = $"SV5T-COL-{DateTime.UtcNow.Year}-{Guid.NewGuid().ToString("N")[..6].ToUpperInvariant()}"
        };
        ...
        {
            var concurrentExisting = await applicationRepository.GetByUnitAndCampaignAsync(campaignId, unitId, tracking: false, ct);
            if (concurrentExisting is not null) return concurrentExisting.Id;
            throw;
        }
    }
    ...
    public async Task SelectCriteriaAsync(Guid applicationId, SelectCriteriaRequest request, CancellationToken ct = default)
    {
        ...
        app.ApplicationCriteria.Clear();
        foreach (var cId in request.CriterionIds.Distinct())
        {
            app.ApplicationCriteria.Add(new ApplicationCriterion
            {
    ...
    public async Task SubmitApplicationAsync(Guid applicationId, CancellationToken ct = default)
    {
        ...
        app.Submit(CurrentUserId); // Rich Domain Behavior
...
{
    [HttpPost("campaigns/{campaignId:guid}/register-individual")]
    public async Task<IActionResult> RegisterIndividual(Guid campaignId, CancellationToken ct)
    {
        var appId = await applicationService.CreateIndividualApplicationAsync(campaignId, ct);
        return Ok(new { applicationId = appId });
    }

    [HttpPost("campaigns/{campaignId:guid}/register-collective")]
    public async Task<IActionResult> RegisterCollective(Guid campaignId, [FromBody] RegisterCollectiveRequest request, CancellationToken ct)
    {
        var appId = await applicationService.CreateCollectiveApplicationAsync(campaignId, request.UnitId, request.UnitName, ct);
        return Ok(new { applicationId = appId });
    }
