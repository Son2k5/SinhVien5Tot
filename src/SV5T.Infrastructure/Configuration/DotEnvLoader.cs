...
public static class DotEnvLoader
{
    ...
    private static string? ResolveEnvPath(string contentRoot)
    {
        ...
        var srcApiDirectory = Path.Combine(contentRoot, "src", "SV5T.Api");
        if (File.Exists(Path.Combine(srcApiDirectory, "SV5T.Api.csproj")))
        {
            return ExistingPath(Path.Combine(srcApiDirectory, ".env"));
        }
